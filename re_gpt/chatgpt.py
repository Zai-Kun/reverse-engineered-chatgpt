...
class ChatGPT:
    ...
    def __init__(self, session_token=None, secure_data_key=None, secure_data_path="data"):
        self.encryption_manager = EncryptionManager(secure_data_key=secure_data_key, secure_data_path=secure_data_path)
    ...
    async def fetch_chat(self, user_id=None, conversation_id=None):
    ...
    async def chat(self, user_id, user_input):
        if user_id not in self.conversations:
            payload = self.build_message_payload(user_input, new_chat=True)
            self.conversations[user_id] = {}
        else:
            if "free" in self.conversations[user_id] and not self.conversations[user_id]["free"]:
                yield False
            ...
            while True:
                ...
                async for chunk in response:
                    ...
                    for line in decoded_chunk.splitlines():
                        ...
                        if "message" in decoded_json and decoded_json["message"]["author"]["role"] == "assistant":
                            yield decoded_json["message"]["content"]["parts"][0]
                            full_message = decoded_json
                if full_message["message"]["metadata"]["finish_details"]["type"] == "max_tokens":
                    conversation_id = full_message["conversation_id"]
                    parent_id = full_message["message"]["id"]

                    payload = self.build_message_continuation_payload(conversation_id, parent_id)
                else:
                    break
        finally:
    ...
    async def send_message(self, payload):
        ...
        async def perform_request():
            ...
            response = await self.session.post(url=url, headers=self.build_request_headers(), json=payload, content_callback=content_callback)
    ...
    async def delete_conversation(self, user_id):
        ...
        response = await self.session.patch(url=url, headers=self.build_request_headers(), json={'is_visible': False})
    ...
    async def update_auth_token(self, session_token):
        ...
        headers = {
            ...
            'Sec-GPC': '1',
            "Cookie": "; ".join([f"{cookie_key}={cookie_value}" for cookie_key, cookie_value in cookies.items()])
        }
    ...
    @staticmethod
    def build_message_payload(user_input, parent_id=None, conversation_id=None, new_chat=True):
        payload = {
    ...
    @staticmethod
    def build_message_continuation_payload(conversation_id, parent_id):
        payload = {
            "action": "continue",
            "arkose_token": None,
            "conversation_id": conversation_id,
